...
pub struct FnReturnCallbackTuner<'rs, TOwner, TArgRefsTuple> {
    _phantom_args_tuple: PhantomData<TArgRefsTuple>,
    fn_config: Arc<RefCell<FnConfig<'rs>>>,
    owner: &'rs TOwner,
}

impl<'rs, TOwner, TArgRefsTuple> FnReturnCallbackTuner<'rs, TOwner, TArgRefsTuple> {
...
impl<'rs, TOwner, TArgRefsTuple> Deref for FnReturnCallbackTuner<'rs, TOwner, TArgRefsTuple> {
    type Target = TOwner;
