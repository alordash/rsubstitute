...
pub enum Arg<T> {
    ...
    #[doc(hidden)]
    PrivateIs(Box<dyn Fn(&T) -> bool>, PrivacyMarker),
}
...
impl<T> Arg<T> {
    ...
    pub fn Is<'a, TFn: Fn(&T) -> bool + 'a>(predicate: TFn) -> Self {
        ...
        return Self::PrivateIs(static_reference, PrivacyMarker);
    }
}
...
impl<'a, T: Debug + ?Sized> Arg<&'a mut T> {
    pub fn check_ref(&self, arg_name: &'static str, actual_value: &&'a mut T) -> ArgCheckResult {
        let arg_info = ArgInfo::new(arg_name, &actual_value);
        let actual_ptr = std::ptr::from_ref(*actual_value);
        match self {
            Arg::Eq(expected_value) => {
