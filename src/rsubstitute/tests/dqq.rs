...
//         }
//     }
//     thread_local! {# [allow (non_upper_case_globals )]static qqwe_MOCK : LazyLock < qqweMock <T> > = LazyLock :: new (| | {let data = Arc :: new (qqweMockData {_phantom_lifetime : PhantomData , base_caller : Arc :: new (RefCell :: new (qqweBaseCaller {_phantom_T : PhantomData })), qqwe_data : FnData :: new ("qqwe" , & SERVICES )}); return qqweMock {setup : qqweMockSetup {data : data . clone ()}, received : qqweMockReceived {data : data . clone ()}, data }; }); }
//     pub fn setup<
//         '__rsubstitute_arg_field_lifetime,
//         T: std::fmt::Debug + core::cmp::PartialOrd + core::clone::Clone,
//     >(
...
//         qqweBaseCaller<T>,
//     > {
//         qqwe_MOCK.as_static().data.qqwe_data.reset();
//         return qqwe_MOCK.as_static().setup.setup(value);
//     }
//     pub fn received<T>(
//         value: impl Into<Arg<'static, T>>,
//         times: Times,
