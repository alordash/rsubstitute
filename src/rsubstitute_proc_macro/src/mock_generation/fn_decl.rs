...
pub(crate) fn extract(
    ...
    let fn_decls = trait_items
        .into_iter()
        .flat_map(|x| try_map_trait_item_fn(ctx, mock_generics, x))
        .collect();
    return fn_decls;
}
...
pub(crate) fn extract_struct_fns(
    ...
    let fn_decls = impl_item_fns
        ...
        .map(|x| map_impl_item_fn(ctx, mock_generics, x))
        .collect();
    return fn_decls;
}
...
fn try_map_trait_item_fn(
    ...
    match trait_item {
        TraitItem::Fn(trait_item_fn) if trait_item_fn.sig.ident != constants::NEW_IDENT.clone() => {
            Some(map_trait_item_fn(ctx, mock_generics, trait_item_fn))
        }
        _ => None,
    }
}
...
fn map_impl_item_fn(ctx: &Ctx, mock_generics: &MockGenerics, impl_item_fn: &ImplItemFn) -> FnDecl {
...
fn create_fn_decl(
    ...
    let maybe_phantom_return_field = try_get_phantom_return_field(&sig.output, &sig.generics);
