...
// TODO - need to check type bounds that refer items in trait generics (this may be heuristic, but may be not, since we're comparing `ident`s of generic arguments, not actual types)
// only type bounds that pass this check must be extracted to result where predicates
pub(crate) fn extract_generics_into_where_predicates(
    ...
    let result = generics
        ...
        .iter_mut()
        .flat_map(|x| match x {
            GenericParam::Lifetime(lifetime_param) => core::mem::take(&mut lifetime_param.bounds)
                .into_iter()
                .map(|lifetime_bound| {
                    let span = lifetime_param.span();
                    WherePredicate::Lifetime(PredicateLifetime {
                        attrs: Vec::new(),
                        lifetime: lifetime_param.lifetime.clone(),
                        colon_token: Token![:](span),
                        bounds: punctuated([lifetime_bound.clone()]),
                    })
                })
                .collect::<Vec<_>>(),
            GenericParam::Type(type_param) => core::mem::take(&mut type_param.bounds)
                .into_iter()
                .map(|type_bound| {
