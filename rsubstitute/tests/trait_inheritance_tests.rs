...
mod __rsubstitute_generated_TraitMock {
    ...
    impl<'__rsa> SuperTraitSetup<'__rsa> {
        pub fn overwork(
            ...
            true,
        > {
            let args_checker = overwork_ArgsChecker::<'__rsa> {
                __rs_generics: ::core::marker::PhantomData,
            };
            let fn_data: &::rsubstitute::for_generated::FnData<
                '_,
...
mod tests {
    ...
    fn test_mocked() {
        // Arrange
        let mut mock = SuperTraitMock::new();
        mock.setup().overwork().returns(10);

        // Act
        let result = mock.overwork();

        // Assert
        assert_eq!(result, 10);
        mock.received().overwork(1.time()).no_other_calls();
    }
    ...
    fn test_base() {
        // Arrange
        let mut mock = SuperTraitMock::new();
        // mock.setup().work().returns(10).overwork().call_base(); // TODO
